...
| Scenario compare (Phase 1–2 API + UI) | Apex: `listScenarios` / `getScenarioCompare` / `createScenario` / `setForecastScenario` (`ScenarioColumn.workingLines` included). Studio **Working changes** is the scenario surface: stacked Option 1 / Option 2 cards (full line table + This add + Finalized per option), Duplicate, Update / Open / Set forecast. Shared **Current** stays once at the top. **Auto-Opportunity** on Studio load when amend Quote has no Opp. **Polish (later):** Delete scenario (confirm; block if forecast-synced). |
