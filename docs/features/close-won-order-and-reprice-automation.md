...
| Scenario compare (Phase 1–2 API) | `listScenarios` / `getScenarioCompare` (read) · `createScenario` (Connect quote-scoped clone + Name/OpportunityId) · `setForecastScenario` (`Opportunity.SyncedQuoteId`). Compare UI + inline qty/disc come next. |
