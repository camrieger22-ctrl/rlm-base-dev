...
| Amendment Studio (Phase 2+) | `rlmAmendmentStudio` embeds on Amend Quotes (`OriginalActionType = Amend`) as the primary workspace after Managed Asset Viewer → `initiateAmendment`. Layout: left **Catalog** (OOTB Discovery composition: `getCatalogs` / `getCategories` chips → `getProducts` browse + `findProducts` search, price-book fallback; Browse Catalogs remains the full-configure escape hatch) · Current / This add / Finalized KPIs · Working changes (qty, discount, start via OOTB Place Sales Transaction line PATCH + FORCE price — not QLI DML when Quote Line Details own sales price; waterfall after reprice + preview while dirty) · **Installed** ledger in a slide-over. Classic TLE/summary hidden on Amend Quotes. Surfaces `ValidationResult` when pricing needs attention. Multi-period / future-dated ASP amends rely on Near Core **Map Line Item** + `enableAdvancedDetailLinePricing` for Quote Line Detail totals. **Next:** multi-scenario compare (sibling amend Quotes + sync one for forecast); decrease/cancel + multi-asset. |
| Scenario compare (Phase 1 API) | `RLM_AssetPriceHistoryController.listScenarios` / `getScenarioCompare` — read-only sibling amend Quotes on an Opportunity (This add / Finalized / Grand Total per quote). Create/sync come in a later PR (Connect quote-scoped clone + `SyncedQuoteId`). |
