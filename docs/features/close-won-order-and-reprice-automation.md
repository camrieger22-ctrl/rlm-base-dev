...
| Scenario compare (Phase 1–2 API + UI) | Apex: `listScenarios` / `getScenarioCompare` / `createScenario` / `setForecastScenario`. Studio **Compare scenarios** panel: side-by-side columns with inline qty/disc Update, Open, Set forecast, Duplicate scenario. |
