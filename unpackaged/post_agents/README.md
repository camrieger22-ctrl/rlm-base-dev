...
| Quoting Assistant agent | `aiAuthoringBundles/RLM_Quoting_Assistant/` | Builder Script `.agent` authoring bundle (developer name `RLM_Quoting_Assistant`; label "Quoting Assistant"). A ground-up, scoped demo quoting agent — tight arc (find products → create/identify quote → add line → discount → configure → totals → **AmendmentStudio** impact/history/DocGen), unified no-redundant-confirmation policy (`require_user_confirmation: False` everywhere), and a names-not-IDs presentation contract (native record cards; Id outputs flagged `is_used_by_planner: false`). Backed by the `RLM_AI_*` helper services/flows below. Intentionally excludes starting asset amend/renew from Assets and usage/consumption; working on an existing amendment quote is in scope via `AmendmentStudio`. |
...
| Quoting Assistant helper services | `classes/RLM_AI_AddProductToQuoteService`, `RLM_AI_ApplyQuoteLineDiscountsService`, `RLM_AI_QuoteDemoSummaryService`, `RLM_AI_AmendImpactSummaryService`, `RLM_AI_AssetPriceHistoryService`, `RLM_AI_CatalogSearchService`, `RLM_AI_GeneratePartnershipDocsService` (+ tests) | Apex invocable services behind Quoting Assistant helper flows: one-call add-product, one-call bulk line discount, names-only quote recap, **amend impact / asset history / catalog search** (wrap close-won services), and **Bears partnership DocGen** (`RLM_DocumentGenerationCreate`, Advanced). Amend helpers depend on `post_closewon` + `post_docgen` classes already in the org. |
